...
function agentes(data) {
    ...
    for (var i = 0; i <data.data.length ; i++) { /* Se ejecuta para cada agente */
        if (data.data[i].bustPortrait !== null) { /* Puede que haya agentes duplicados sin foto, aquí se evita */
            var divpersonaje = document.createElement("div");
            divpersonaje.classList.add("divpersonaje");
            divpersonaje.id = i; /* Asignamos un id */

            divpersonaje.addEventListener("click", function() { /* Al hacer click, ejecutamos la función */
